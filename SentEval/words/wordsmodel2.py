...
class Statistician(nn.Module):
    def __init__(self, batch_size=16, n_features=1,
        ...
        # modules
        # statistic network
        statistic_args = (self.batch_size, self.n_features,
        ...
        z_args = (self.batch_size, self.n_features,
        ...
        # observation decoder
        observation_args = (self.batch_size, self.n_features,
    ...
    def forward(self, x, sample_size):
        ...
        x_mean, x_logvar = self.observation_decoder(zs, c, sample_size)
    ...
    def loss(self, outputs, weight, sample_size):
        ...
        recon_loss = gaussian_log_likelihood(x.view(-1, self.n_features),
                                             x_mean, x_logvar)
        recon_loss /= (self.batch_size * sample_size)
        ...
        shapes = (
            (self.batch_size, sample_size, self.z_dim),
    ...
    def step(self, inputs, alpha, optimizer, clip_gradients=True, sample_size):
        assert self.training is True

        outputs = self.forward(inputs, sample_size)
        loss, vlb = self.loss(outputs, weight=(alpha + 1), sample_size)
    ...
    def sent_embedding(self, dataset):
        c1, v1 = self.statistic_network(dataset, summarize=True)
        return torch.cat([c1,v1])
