...
class StatisticNetwork(nn.Module):
    ...
    def pool(self, x):
        e = x.mean(1).view(-1, self.n_features)
        return e
...
class InferenceNetwork(nn.Module):
	...
	ex = self.pool(x)
        ex = self.fc_x(ex)
        ex = ex.view(self.batch_size, 1, self.hidden_dim)
...
class ObservationDecoder(nn.Module):
    ...
    def __init__(self, batch_size, n_features,
        ...
        # modules
        self.fc_zs = nn.Linear(self.n_stochastic * self.z_dim, self.hidden_dim*2)
        self.fc_c = nn.Linear(self.c_dim, self.hidden_dim*2)
    ...
    def forward(self, zs, c, sample_size):
        ...
        ec = self.fc_c(c)
        ec = ec.view(self.batch_size, 1, self.hidden_dim*2).expand_as(ezs)
